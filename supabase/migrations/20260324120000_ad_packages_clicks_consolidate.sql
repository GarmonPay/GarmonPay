-- Single catalog: remove duplicate legacy seeds (starter/creator vs basic_reach, etc.),
-- add click credits + display order, and seed profit-safe view+click packages.

ALTER TABLE public.ad_packages
  ADD COLUMN IF NOT EXISTS included_clicks INTEGER NOT NULL DEFAULT 0;
...
-- Economics: up to $0.01/view + $0.05/click to members. Engagements bill advertisers at 2× member payout
-- (see /api/ads/engage). Allotments are chosen so price >= 2 × member pool + headroom.
