-- GARMONPAY ARENA — Full schema. Economy rules enforced in application layer.
...
comment on column public.users.arena_coins is 'One-way currency for Arena store/cosmetics; cannot be withdrawn.';
...
-- Arena transactions (wallet movements for arena)
...
-- Admin earnings (all revenue streams)
...
-- Arena coins balance (denormalized from users.arena_coins; optional ledger)
...
-- Spectators (who is watching a fight)
...
-- RLS: service role only for arena tables (app uses service role for arena APIs)
...
-- Service role (SUPABASE_SERVICE_ROLE_KEY) bypasses RLS by default in Supabase.
-- Add user-facing RLS policies when exposing arena tables to anon/authenticated.
