-- C-Lo: allow player stake from room minimum up to current bank; snapshot bank at round start for settlement.

ALTER TABLE public.celo_rounds
  ADD COLUMN IF NOT EXISTS bank_at_round_start_sc integer;

COMMENT ON COLUMN public.celo_rounds.bank_at_round_start_sc IS
  'Table bank (GPC) when the round started; used with winning stake to detect full bank-cover / stop-the-bank.';
...
BEGIN
  ...
  v_min_entry := GREATEST(
    500,
    COALESCE(v_room.minimum_entry_sc, v_room.min_bet_cents, 100)
  );
  v_max_bank := GREATEST(
    0,
    COALESCE(v_room.current_bank_sc, v_room.current_bank_cents, 0)::integer
  );

  IF p_amount < v_min_entry THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bet must be at least the room minimum.');
  END IF;
  IF v_max_bank <= 0 THEN
    RETURN jsonb_build_object('success', false, 'message', 'Table bank is empty.');
  END IF;
  IF p_amount > v_max_bank THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bet cannot exceed the current bank.');
  END IF;
...
COMMENT ON FUNCTION public.celo_post_entry_atomic(uuid, uuid, integer, text, text, text)
  IS 'Atomically debits GPC, inserts coin_transactions, and marks celo_room_players as posted entry. Stake must be between room minimum and current bank.';
