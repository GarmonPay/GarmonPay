-- Stake & Escape: server-authoritative sessions, puzzles, payouts, admin (service-role APIs).
create extension if not exists pgcrypto;

alter table public.users add column if not exists kyc_verified boolean not null default false;
...
create index if not exists escape_room_sessions_player_idx on public.escape_room_sessions (player_id, started_at desc);
create index if not exists escape_room_sessions_window_idx on public.escape_room_sessions (prize_pool_window, mode, result);
create index if not exists escape_room_sessions_active_idx on public.escape_room_sessions (result, started_at desc) where result = 'active';
create index if not exists escape_room_sessions_suspicious_idx on public.escape_room_sessions (suspicious, started_at desc) where suspicious = true;
...
create index if not exists escape_room_flags_status_idx on public.escape_room_flags (status, created_at desc);
...
create index if not exists escape_room_timer_logs_session_idx on public.escape_room_timer_logs (session_id, server_time desc);
...
create index if not exists escape_room_payouts_status_idx on public.escape_room_payouts (status, created_at desc);
...
create trigger trg_escape_room_settings_updated_at
  ...
  for each row execute procedure public.escape_room_set_updated_at();
...
where not exists (
  select 1 from public.escape_room_puzzles
  where active_date = (current_timestamp at time zone 'utc')::date and is_active = true
);
