...
/** Inline-only colors — no shared asset modules. */
function gloveColor(fighter?: Record<string, unknown> | null): string {
  const g = String(
    fighter?.equipped_gloves_key ?? fighter?.equipped_gloves ?? ""
  ).toLowerCase();
  if (g.includes("championship")) return "#f0c040";
  if (g.includes("titanium")) return "#c0c0c0";
  if (g.includes("pro_gloves") || (g.includes("pro") && g.includes("glove")))
    return "#1a3a8f";
...
function shortsColor(fighter?: Record<string, unknown> | null): string {
  const s = String(
    fighter?.equipped_shorts_key ?? fighter?.equipped_shorts ?? ""
  ).toLowerCase();
...
function skinColor(fighter?: Record<string, unknown> | null): string {
...
export default function FighterLayers({
  ...
  const gc = gloveColor(fighter);
  const sc = shortsColor(fighter);
  const sk = skinColor(fighter);
  const fc = String(fighter?.fighter_color ?? "#f0a500");
    ...
    >
      <div
        style={{
          ...
          height: "100%",
          background:
            "radial-gradient(ellipse at 50% 0%, rgba(255,245,224,0.15) 0%, transparent 70%)",
          pointerEvents: "none",
        }}
      />
      <svg
      ...
      >
        ...
        <rect x="62" y="190" width="20" height="58" rx="8" fill={sk} />
        <rect x="100" y="195" width="20" height="56" rx="8" fill={sk} />
        <rect x="56" y="178" width="68" height="42" rx="6" fill={sc} />
        ...
        <rect x="60" y="105" width="60" height="78" rx="10" fill={sk} />
        <path
          d="M 68 118 Q 85 110 90 118 Q 90 128 68 128 Z"
          fill="rgba(0,0,0,0.08)"
        />
        <path
          d="M 112 118 Q 95 110 90 118 Q 90 128 112 128 Z"
          fill="rgba(0,0,0,0.08)"
        />
        ...
        <rect x="82" y="88" width="16" height="20" rx="5" fill={sk} />
        <ellipse cx="90" cy="72" rx="24" ry="28" fill={sk} />
        <ellipse cx="90" cy="88" rx="18" ry="10" fill={sk} />
        ...
        <path
          ...
          strokeLinecap="round"
        />
        <ellipse cx="66" cy="72" rx="5" ry="7" fill={sk} />
        <ellipse cx="114" cy="72" rx="5" ry="7" fill={sk} />
        <rect
          x="36"
          y="108"
          width="18"
        ...
        />
        <ellipse cx="28" cy="98" rx="18" ry="14" fill={gc} />
        <ellipse cx="28" cy="90" rx="14" ry="8" fill={gc} />
        <ellipse
          cx="14"
          cy="95"
          rx="7"
        ...
        />
        <ellipse cx="138" cy="95" rx="16" ry="13" fill={gc} />
        <ellipse cx="138" cy="88" rx="12" ry="7" fill={gc} />
        <ellipse
          cx="152"
          cy="93"
          rx="6"
