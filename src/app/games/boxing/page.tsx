...
export default function BoxingGamePage() {
  ...
  if (!WS_URL) {
    return (
      <div className="min-h-screen bg-[#0f172a] p-4 flex flex-col items-center justify-center gap-4">
        <h1 className="text-2xl font-bold text-white">Boxing Arena</h1>
        <p className="text-white/80 text-center max-w-md">
          Set <code className="bg-white/10 px-1 rounded">NEXT_PUBLIC_BOXING_WS_URL</code> to your fight server URL (e.g. <code className="bg-white/10 px-1 rounded">http://localhost:4000</code>) to enable online matchmaking.
        </p>
        <Link href="/dashboard/games/boxing" className="text-fintech-accent hover:underline text-sm">
          ← Back to games
        </Link>
      </div>
    );
