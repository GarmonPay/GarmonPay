  ...
  if (inFight) {
    ...
    return (
      <div className="min-h-[85vh] flex flex-col rounded-xl bg-[#161b22] border border-white/10 overflow-hidden">
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-around", alignItems: "flex-end" }} className="gap-2">
            <div className="flex-1 min-w-0 max-w-[48%] flex flex-col items-center">
              {healthA < 100 && (
                <div className="w-full max-w-[140px] h-2 bg-[#1f2937] rounded-full overflow-hidden mb-1">
                  <div className="h-full bg-[#22c55e]" style={{ width: `${Math.max(0, healthA)}%` }} />
                </div>
              )}
              <ProBoxer facingRight={true} fighterColor="#f0a500" size="medium" />
              <span className="text-xs text-[#9ca3af] mt-1 text-center">{fa?.name ?? "You"}</span>
            </div>
            <div className="flex-1 min-w-0 max-w-[48%] flex flex-col items-center">
              {healthB < 100 && (
                <div className="w-full max-w-[140px] h-2 bg-[#1f2937] rounded-full overflow-hidden mb-1">
              ...
              <span className="text-xs text-[#9ca3af] mt-1 text-center">{fb?.name ?? "Opponent"}</span>
            </div>
          </div>
        </div>
        <div className="p-4 border-t border-white/10 flex-1">
          ...
          {winnerId != null && (
            <div className="mb-4 p-4 rounded-lg bg-[#0d1117] border border-white/10">
              ...
              <Link
                ...
                className="inline-block mt-2 text-[#f0a500] hover:underline"
                onClick={() => {
                  setFightId(null);
                  setJoinToken(null);
                  setFighterA(null);
                  ...
                  setPendingAiFight(null);
                  setShowPreFight(false);
                  setLoserKoActive(false);
                  setOpponentKoActive(false);
                }}
              >
                Fight again
              </Link>
              <Link href="/dashboard/arena" className="inline-block mt-2 ml-4 text-[#f0a500] hover:underline">
                Back to Arena
              </Link>
            </div>
          )}
          ...
          {log.length > 0 && (
            <div className="mt-2 max-h-24 overflow-y-auto rounded bg-[#0d1117] p-2 text-xs text-[#9ca3af]">
              {log.slice(-6).map((e, i) => (
                <div key={i}>
                  You: {e.actionA} → {e.damageAtoB} dmg {e.hitA && "✓"} · Opp: {e.actionB} → {e.damageBtoA} dmg{" "}
                  {e.hitB && "✓"}
                </div>
              ))}
            </div>
          ...
          <div className="mt-3">
            <Link href="/dashboard/arena" className="text-[#f0a500] hover:underline">
              Back to Arena
            </Link>
          </div>
        </div>
