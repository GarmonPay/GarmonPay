...
export default function ArenaAchievementsPage() {
  ...
  const fetchAchievements = useCallback(async () => {
    ...
    const res = await fetch(`${API_BASE}/arena/achievements`, { headers, credentials: "include" });
  ...
  const checkNow = async () => {
    ...
    await fetch(`${API_BASE}/arena/achievements/check`, { method: "POST", headers, credentials: "include", body: "{}" });
    fetchAchievements();
    setChecking(false);
  };
  ...
  return (
    <div className="rounded-xl bg-[#161b22] border border-white/10 p-6 max-w-2xl">
      ...
      {weightClass && <p className="text-[#9ca3af] text-sm mb-4">Your weight class: <span className="text-white">{weightClass}</span> (matchmaking uses this)</p>}
